...
/**
  ...
  */
final class TableCT(X: Array[CPIntVar], table: Array[Array[Int]]) extends Constraint(X(0).store, "CT") {
...
//    print(" propagate start .......... "+"\n")
    ...
    while (j > 0) {
      ...
      {
      val maxValue = X(varIndex).max
      val supCountSize = maxValue+1
      val supCount = Array.ofDim[Int](supCountSize)
//      if ((nChanged > 1 || changedVarIdx != varIndex) && !x (varIndex).isBound) {
        ...
        while (i < domainArraySize) {
          ...
          val propCount = validTuples.intersectCount (variableValueSupports (varIndex)(value))
          supCount(value + offsets(varIndex)) = propCount
          print ("In propagate, variable " + X(varIndex).hashCode() + " propCount "  + propCount + " Domain : " + X(varIndex).toList + " supCount " + supCount.toList +"\n")
//          print ( " offset " + offsets(varIndex)+"\n")
        }

      if(supCount.max >0){
        print ("In propagate, variable " + X(varIndex).hashCode() + " Domain : " + X(varIndex).toList + "consID" + getConsID + " supCount " + supCount.toList +"\n")
        x(varIndex).changeCount(supCount, X(varIndex).hashCode(), getConsID) //getConsID is from the constraint in cp.core.modelling
      }
      }
      if (x (varIndex).isBound) {
...
//    print(" propagate finished .......... "+"\n")
  ...
  @inline private def computeSupportsAndInitialFiltering(valids: ArrayBuffer[Int]): Unit = {
    ...
    for {
      ...
      val supCountSize = maxValue+1
      val supCount = Array.ofDim[Int](supCountSize)
      for {valueIndex <- variableValueSupports (varIndex).indices}
      {
        if (varValueSupports (varIndex)(valueIndex).nonEmpty) {
          ...
          val initCount = validTuples.intersectCount (variableValueSupports (varIndex)(valueIndex))
          supCount(valueIndex+offsets(varIndex)) = initCount
//          print ( " offset " + offsets(varIndex)+"\n")
        } else {
      ...
      if(supCount.max >0) {
        print ("In initial filtering, variable " + X(varIndex).hashCode() + " Domain : " + X(varIndex).toList + "consID" + getConsID + " supCount " + supCount.toList +"\n")
        x(varIndex).changeCount(supCount, X(varIndex).hashCode(), getConsID)
      }
    }
  }
}
